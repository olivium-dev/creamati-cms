...
const ParceletsList: React.FC = () => {
  ...
  const advanceMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      // No need to refresh - advanceParcelet already updates local state
    },
    onError: (error: any) => {
  ...
  const createShipmentMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      // Refresh the data to show the new shipment
  ...
  const handleResetClick = () => {
    setConfirmDialog({
      ...
      message: 'This will clear the local cache and reload fresh data from the delivery gateway.',
      parceletId: 0,
      action: 'reset'
    });
  };
